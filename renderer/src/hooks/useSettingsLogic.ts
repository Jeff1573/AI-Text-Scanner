...
export const useSettingsLogic = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const isValidAccelerator = (acc: string): boolean => {
      if (!acc) return false;
      const part =
        "(CommandOrControl|Command|Control|Ctrl|Cmd|Alt|Option|Shift|Super|Win|Meta|F([1-9]|1[0-9]|2[0-4])|[A-Z0-9]|Space|Tab|Backspace|Delete|Insert|Return|Enter|Escape|Esc|Up|Down|Left|Right|Home|End|PageUp|PageDown)";
    ...
    if (
      !formData.screenshotHotkey ||
      !isValidAccelerator(formData.screenshotHotkey)
    ) {
      newErrors.screenshotHotkey =
        "请输入有效快捷键，如 CommandOrControl+Shift+S";
    }

    if (
      formData.resultHotkey &&
      formData.screenshotHotkey &&
  ...
  // 保存设置
  const handleSaveSettings = async (): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (currentConfig.success) {
        try {
          const autoLaunchResult =
            await window.electronAPI.setLoginItemSettings(formData.autoLaunch);
          if (!autoLaunchResult.success || !autoLaunchResult.verified) {
            setFieldError(
              "autoLaunch",
              autoLaunchResult.error || "开机自启设置失败"
            );
            return { success: false };
          }
          ...
          setFieldError(
            "autoLaunch",
            `开机自启设置异常: ${
              autoLaunchError instanceof Error
                ? autoLaunchError.message
                : String(autoLaunchError)
            }`
          );
          return { success: false };
        }
